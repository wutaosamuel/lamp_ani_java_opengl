...
public class Anilamp_GLEventListener implements GLEventListener {
  ...
  // update the lamp and spotligh move
  ...
  // set random angle of base_lowarm and Higharm
  public void reSet() {
    resetState = true;
  }
  ...
  // set max movement and angle
  public void jumpAnimation() {
    boolean jumpState = true;
    float xRandom = (float)Math.random();
    float zRandom = (float)Math.random();
    currentXPosition = lampXPosition;
    currentYPosition = 0;
    currentZPosition = lampZPosition;
    currentBL_angle = thetaBL_angle;
    currentJ_angle = thetaJ_angle;
    while(jumpState)
    {
      maxXPosition = (float)((xRandom - 0.5f) * (tableLength - baseLength)) - currentXPosition;
      maxYPosition = 0.5f * (float)Math.sqrt((double)(maxXPosition*maxXPosition + maxZPosition*maxZPosition)) - currentYPosition;
      maxZPosition = (float)((zRandom - 0.5f) * (tableWidth - baseLength)) - currentZPosition;
      maxBL_angle = (angleLimit-30f-currentBL_angle) * (maxYPosition / (tableLength - baseLength));
      maxJ_angle = (angleLimit - currentJ_angle) * (maxYPosition / (tableLength - baseLength));
      // avoid touch tabletop objects
      if (-0.5f*tableLength <= maxXPosition && maxXPosition <= (-0.5f*tableLength + 3f) && -0.5f*tableWidth <= maxZPosition && maxZPosition <= (-0.5f*tableWidth + 2f))
        jumpState = true;
      if (0.5f*tableLength <= maxXPosition && maxXPosition <= (0.5f*tableLength - 1f) && 0.5f*tableWidth <= maxZPosition && maxZPosition <= (0.5f*tableWidth - 1f))
        jumpState = true;
      jumpState = false;
    }
  ...
  // when change angle, update each angle
  ...
  private Light3Model woodCube, baseCube, metSphere, metalCube, cCube, bookCube, metalSphere, earthSphere;
  private Light3Model wallLow, wallHigh, wallLeft, wallRight, floor, window;
  ...
  private float maxBL_angle = 0f;
  private float maxJ_angle = 0f;
  private float currentBL_angle = 30f;
  private float currentJ_angle = 30f;
  ...
  private float junctionScale = 1f;
  ...
  private void initialise(GL3 gl) {
    ...
    int[] textureId12 = TextureLibrary.loadTexture(gl, "textures/ear0xuu2.jpg");
    int[] textureId13 = TextureLibrary.loadTexture(gl, "textures/pink.jpg");
    int[] textureId14 = TextureLibrary.loadTexture(gl, "textures/ven0aaa2.jpg");
    int[] textureId15 = TextureLibrary.loadTexture(gl, "textures/ven0aaa2_specular.jpg");
    int[] textureId16 = TextureLibrary.loadTexture(gl, "textures/black.jpg");
    int[] textureId17 = TextureLibrary.loadTexture(gl, "textures/white.jpg");
    ...
    Mat4 cubeModelMatrix = Mat4.multiply(Mat4Transform.scale(1,1,1), Mat4Transform.translate(0,0.5f,0));
     ...
     Material woodMaterial = new Material(new Vec3(1.0f, 0.5f, 0.31f), new Vec3(1.0f, 0.5f, 0.31f), new Vec3(0.5f, 0.5f, 0.5f), 32.0f);
      ...
      baseCube = new Light3Model(gl, camera, light1, light2, spotLight, cubeShader, woodMaterial, cubeModelMatrix, cubeMesh, textureId14, textureId5);
      bookCube = new Light3Model(gl, camera, light1, light2, spotLight, cubeShader, woodMaterial, cubeModelMatrix, cubeMesh, textureId5, textureId6);
      cCube = new Light3Model(gl, camera, light1, light2, spotLight, cubeShader, woodMaterial, cubeModelMatrix, cubeMesh, textureId3, textureId4);
    ...
    Mat4 sphereModelMatrix = Mat4.multiply(Mat4Transform.scale(1,1,1), Mat4Transform.translate(0,0.5f,0));
     ...
     // Sphere::matelMaterial(had)
      ...
      metalSphere = new Light3Model(gl, camera, light1, light2, spotLight, sphereShader, metalMaterial, sphereModelMatrix, sphereMesh, textureId14, textureId15);
      metSphere = new Light3Model(gl, camera, light1, light2, spotLight, sphereShader, metalMaterial, sphereModelMatrix, sphereMesh, textureId10,textureId17);
      earthSphere = new Light3Model(gl, camera, light1, light2, spotLight, sphereShader, woodMaterial, sphereModelMatrix, sphereMesh, textureId12, textureId17);
    ...
    Shader backgrdShader = new Shader(gl, "vs_3light.txt", "fs_3light.txt");
    // Background::floor
      ...
      Mat4 floorMatrix = Mat4Transform.scale(1,1f,1);
        floorMatrix = Mat4.multiply(floorMatrix, Mat4Transform.scale(floorSize,1f,floorSize));
          floor = new Light3Model(gl, camera, light1, light2, spotLight, backgrdShader, floorMaterial, floorMatrix, backgrdMesh, textureId0);
    // Background::window
      ...
      Mat4 windowMatrix = Mat4Transform.scale(1,1f,1);
        ...
        windowMatrix = Mat4.multiply(Mat4Transform.translate(0, wallHight1+windowSize*0.5f,-floorSize*0.5f), windowMatrix);
          window = new Light3Model(gl, camera, light1, light2, spotLight, backgrdShader, windowMaterial, windowMatrix, backgrdMesh, textureId11, textureId17);
    // Background::wall
      ...
      Mat4 wallMatrix = Mat4Transform.scale(1,1f,1);
        ...
        wallMatrix = Mat4.multiply(Mat4Transform.translate(0, wallHight1*0.5f, -floorSize*0.5f), wallMatrix);
          wallLow = new Light3Model(gl, camera, light1, light2, spotLight, backgrdShader, wallMaterial, wallMatrix, backgrdMesh, textureId13, textureId17);
        ...
        wallMatrix = Mat4.multiply(Mat4Transform.translate(0, wallHight1*1.5f + windowSize, -floorSize*0.5f), wallMatrix);
          wallHigh = new Light3Model(gl, camera, light1, light2, spotLight, backgrdShader, wallMaterial, wallMatrix, backgrdMesh, textureId13, textureId17);
        ...
        wallMatrix = Mat4.multiply(Mat4Transform.translate(wallWidth2*0.5f-wallWidth1*0.5f, wallHight1 + windowSize*0.5f, -floorSize*0.5f), wallMatrix);
          wallLeft = new Light3Model(gl, camera, light1, light2, spotLight, backgrdShader, wallMaterial, wallMatrix, backgrdMesh, textureId13, textureId17);
        ...
        wallMatrix = Mat4.multiply(Mat4Transform.translate(wallWidth1*0.5f - wallWidth2*0.5f, wallHight1 + windowSize*0.5f, -floorSize*0.5f), wallMatrix);
          wallRight = new Light3Model(gl, camera, light1, light2, spotLight, backgrdShader, wallMaterial, wallMatrix, backgrdMesh, textureId13, textureId17);
    ...
    NameNode phone = new NameNode("phone");
      tableM = new Mat4(1);
      tableM = Mat4.multiply(tableM, Mat4Transform.translate(0.5f*tableLength-1f, tableHeight, 0.5f*tableWidth-1f));
      tableM = Mat4.multiply(tableM, Mat4Transform.scale(2f,2f,2f));
      tableM = Mat4.multiply(tableM, Mat4Transform.translate(0,0.5f,0));
      TransformNode phoneTransform = new TransformNode("phone Transform", tableM);
        ModelNode phoneShape = new ModelNode("Cube(phone)", cCube);

    NameNode studentCard = new NameNode("studentCard");
      tableM = new Mat4(1);
      tableM = Mat4.multiply(tableM, Mat4Transform.translate(0.5f*tableLength-2.5f, tableHeight, -0.5f*tableWidth+0.2f));
      tableM = Mat4.multiply(tableM, Mat4Transform.scale(1f,0.1f,2f));
      tableM = Mat4.multiply(tableM, Mat4Transform.translate(0,0.5f,0));
      TransformNode studentCardTransform = new TransformNode("studentCard Transform", tableM);
        ModelNode studentCardShape = new ModelNode("Cube(phoe)", bookCube);

      NameNode earth = new NameNode("earth");
      tableM = new Mat4(1);
      tableM = Mat4.multiply(tableM, Mat4Transform.translate(0.5f*tableLength-1.0f, tableHeight, -(0.5f*tableWidth-1.0f)));
      ...
      TransformNode earthTransform = new TransformNode("earth Transform", tableM);
        ModelNode earthShape = new ModelNode("Sphere(earth)", earthSphere);
    ...
    NameNode base = new NameNode("base");
      ...
      TransformNode baseTransform = new TransformNode("base transform", lampM);
        ModelNode baseShape = new ModelNode("Cube(base)", baseCube);
    ...
    NameNode lowArm = new NameNode("lowArm");
      ...
      TransformNode lowArmTransform = new TransformNode("lowArm transform", lampM);
        ModelNode lowArmShape = new ModelNode("Sphere(lowArm)", metSphere);
    ...
    NameNode highArm = new NameNode("highArm");
      ...
      TransformNode highArmTransform = new TransformNode("highArm transform", lampM);
        ModelNode highArmShape = new ModelNode("Sphere(highArm)", metSphere);
    ...
    NameNode headL = new NameNode("headL");
      ...
      TransformNode headLTransform = new TransformNode("headL transform", lampM);
        ModelNode headLShape = new ModelNode("Sphere(headL)", baseCube);
  ...
  public void randomJump() {
    ...
    {
      thetaBL_angle = maxBL_angle*(float)Math.sin(elapsedTime) + currentBL_angle;
      thetaJ_angle = -maxJ_angle*(float)Math.sin(elapsedTime) + currentJ_angle;
      thetaBL = (float)(thetaBL_angle * Math.PI/180.0);
      thetaJ = (float)(thetaJ_angle * Math.PI/180.0);
    } else if ((double)(Math.PI/2.0) <= elapsedTime && elapsedTime <= (double)(Math.PI))
    {
      lampXPosition = -maxXPosition*(float)Math.cos(elapsedTime) + currentXPosition;
      lampYPosition = -maxYPosition*(float)Math.sin(elapsedTime * 2.0) + currentYPosition;
      lampZPosition = -maxZPosition*(float)Math.cos(elapsedTime) + currentZPosition;
      thetaBL_angle = -maxBL_angle*(float)Math.sin(elapsedTime) + currentBL_angle;
      thetaJ_angle = maxJ_angle*(float)Math.sin(elapsedTime) + currentJ_angle;
      thetaBL = (float)(thetaBL_angle * Math.PI/180.0);
      thetaJ = (float)(thetaJ_angle * Math.PI/180.0);
  ...
  // update world light1 and light effect
  ...
  // update world light2 and light effect
  ...
  // update model when switch state of lamp has changed
  public void shapeLampState(float switchState) {
    ...
    bookCube.setSwitchLampState(switchState);
    cCube.setSwitchLampState(switchState);
    baseCube.setSwitchLampState(switchState);
    metalSphere.setSwitchLampState(switchState);
    metSphere.setSwitchLampState(switchState);
    earthSphere.setSwitchLampState(switchState);
  ...
  // update model when switch state of world has changed
  public void shapeWorldState(float switchState, PointLight light) {
    ...
    cCube.setSwitchWorldState(switchState, light);
    baseCube.setSwitchWorldState(switchState, light);
    bookCube.setSwitchWorldState(switchState, light);
    metalSphere.setSwitchWorldState(switchState, light);
    metSphere.setSwitchWorldState(switchState, light);
    earthSphere.setSwitchWorldState(switchState, light);
