...
public class mylamp_GLEventListener implements GLEventListener {
  ...
  private Model floor, wall, window, woodCube, metalCube, metalSphere;
  ...
  private void initialise(GL3 gl) {
    ...
    Shader cubeShader = new Shader(gl, "vs_cube_04.txt", "fs_cube_04.txt");
    Mat4 cubeModelMatrix = Mat4.multiply(Mat4Transform.scale(1,1,1), Mat4Transform.translate(0,0.5f,0));
     ...
     Material woodMaterial = new Material(new Vec3(1.0f, 0.5f, 0.31f), new Vec3(1.0f, 0.5f, 0.31f), new Vec3(0.5f, 0.5f, 0.5f), 32.0f);
      ...
      woodCube = new Model(gl, camera, light1, cubeShader, woodMaterial, cubeModelMatrix, cubeMesh, textureId7, textureId8);
     ...
     Material metalMaterial = new Material(new Vec3(0.19225f, 0.19225f, 0.19225f), new Vec3(0.50754f, 0.50754f, 0.50754f), new Vec3(0.508273f, 0.508273f, 0.508273f), 32f*0.4f);
      ...
      metalCube = new Model(gl, camera, light1, cubeShader, metalMaterial, cubeModelMatrix, cubeMesh, textureId9, textureId10);

    // Sphere
    // Sphere::(sphereMesh & sphereShader & sphereModelMatrix)
    Mesh sphereMesh = new Mesh(gl, Sphere.vertices.clone(), Sphere.indices.clone());
    Shader sphereShader = new Shader(gl, "vs_cube_04.txt", "fs_cube_04.txt");
    Mat4 sphereModelMatrix = Mat4.multiply(Mat4Transform.scale(1,1,1), Mat4Transform.translate(0,0.5f,0));
     ...
     // Sphere::matelMaterial(had)
      ...
      metalSphere = new Model(gl, camera, light1, sphereShader, metalMaterial, sphereModelMatrix, sphereMesh, textureId1, textureId2);
    ...
    Shader backgrdShader = new Shader(gl, "vs_tt_05.txt", "fs_tt_05.txt");
    // Background::floor
      ...
      Mat4 floorMatrix = Mat4Transform.scale(1,1f,1);
        floorMatrix = Mat4.multiply(floorMatrix, Mat4Transform.scale(floorSize,1f,floorSize));
          floor = new Model(gl, camera, light1, backgrdShader, floorMaterial, floorMatrix, backgrdMesh, textureId1);
    // Background::window
      ...
      Mat4 windowMatrix = Mat4Transform.scale(1,1f,1);
        ...
        windowMatrix = Mat4.multiply(Mat4Transform.translate(0, wallSize*0.5f,0.1f-wallSize*0.5f), windowMatrix);
          window = new Model(gl, camera, light1, backgrdShader, windowMaterial, windowMatrix, backgrdMesh, textureId11);
    // Background::wall
      ...
      Mat4 wallMatrix = Mat4Transform.scale(1,1f,1);
        wallMatrix = Mat4.multiply(Mat4Transform.scale(wallSize,1f,wallSize), wallMatrix);
        wallMatrix = Mat4.multiply(Mat4Transform.rotateAroundX(90), wallMatrix);
        wallMatrix = Mat4.multiply(Mat4Transform.translate(0, wallSize*0.5f, -wallSize*0.5f), wallMatrix);
          wall = new Model(gl, camera, light1, backgrdShader, wallMaterial, wallMatrix, backgrdMesh, textureId0);
  ...
  private void render(GL3 gl) {
    ...
    light1.setPosition(getLightPosition(15f));  // changing light position each frame
    light1.render(gl);
    light2.setPosition(getLightPosition(5f));
    light2.render(gl);
    floor.render(gl);
    wall.render(gl);
    window.render(gl);
